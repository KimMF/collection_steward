...
- Allow an administrator to create `WBS-intake` without storing or sharing a
  password in SQL or source files.
...
## Create the WBS-intake account

After the migration and files are deployed:

1. Sign in with the existing Kim account.
2. Open **Users**.
3. Leave the suggested username `WBS-intake` and display name `WBS Intake`, or
   change them before saving.
4. Leave **Access** set to **Intake only**.
5. Enter and confirm a password of at least 12 characters.
6. Select **Create account**.
7. Give Sonya the username and password through the normal private method used
   for WBS accounts. Do not add the password to GitHub or project notes.
...
1. Sign out and sign in as `WBS-intake`.
2. Confirm that View assets and Intake are visible.
3. Confirm that Production checkout and Users are not visible.
4. Open `/checkout.php` directly and confirm that access is denied.
5. Add a test donation with at least two tags and a saved photograph.
6. Confirm that the completed item card shows the photograph, permanent asset
   ID, type, size or color when entered, location, and selected tags.
7. Select **Enter another item** and confirm that a blank intake form appears.
